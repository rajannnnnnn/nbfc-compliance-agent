...
## ADR-045 — `contact_datetime`'s 0.0 was a ground-truth bug, not an extraction bug

**Context.** `contact_datetime` scored `exact_match=0.0` across every `call_transcript` case
in both ADR-042 and the follow-up re-run, despite ADR-042's `IST`-suffix fix to
`normalise_time`. A direct live call confirmed the extraction pipeline itself is correct:
`extract_raw_fields()` returns `value_raw='18/05/2026 14:00 IST'`, and
`parse_full_datetime()` parses it to the correct, timezone-aware
`2026-05-18 14:00:00+05:30` — no error, no wrong value.

The mismatch was in this eval suite's own ground truth. `scripts/gen_synthetic_docs.py`'s
`_gen_call_transcript` built its `contact_datetime` fact with
`datetime.combine(base, dtime(contact_hour, 0)).isoformat()` — a **naive** datetime, no
timezone. `fields.yaml` declares `contact_datetime`'s normalisation as `timezone:
Asia/Kolkata, assume_local: true`, and the real pipeline (correctly) produces a
timezone-**aware** `+05:30` datetime. `value_to_jsonable` serialises these two differently
(`"2026-05-18T14:00:00"` vs `"2026-05-18T14:00:00+05:30"`), so the eval comparison failed on
every single case — not because the extraction was ever wrong, but because the ground truth
generator didn't match the field's own documented contract.

**Decision.** `_gen_call_transcript` now builds `contact_datetime` with `tzinfo=_IST`
(`ZoneInfo("Asia/Kolkata")`), matching `fields.yaml`'s contract exactly. Fixtures regenerated
with the same seed (`scripts/gen_synthetic_docs.py --seed 42`) — deterministic, so only the
25 `call_transcript` `.facts.json` files' `contact_datetime` value changed; every other
fixture and all `.txt` document text is byte-identical. `eval/cases/extraction_core/*.json`
regenerated from the corrected fixtures via `scripts/gen_extraction_eval_cases.py`.

**Why this matters beyond one field.** This is a caution about the whole extraction_core
methodology, not just one field: ground truth generated by the same kind of Python code that
renders the fixture is only as correct as that code's own fidelity to the field registry's
documented contract. A field-by-field audit of `scripts/gen_synthetic_docs.py`'s facts
against `app/schema/fields.yaml`'s `normalisation` specs would be worth doing before trusting
any other field's score at face value — this one was caught only because its score was an
implausible, suspicious 0.0 across every case, not because of a systematic check.

**Verification.** `tests/unit/test_synthetic_docs.py` (6 tests, including the field-set
