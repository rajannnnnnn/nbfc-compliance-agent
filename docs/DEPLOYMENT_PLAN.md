...
| API (FastAPI/Uvicorn) | **Fly.io** (1 shared-cpu-1x VM) | Runs a real long-lived process (needed for the Celery worker anyway — see below), predictable | Fly's free allowance is small and requires a card on file — not unconditionally $0, but effectively $0 at this scale. Alternative: Render free web service, but it cold-starts after 15 min idle, which conflicts with release gate #1 ("no cold start") |
| Celery worker | **Same Fly.io VM as the API**, second process | Avoids paying for a second machine; Fly supports multiple processes per app via `[processes]` in `fly.toml` | None beyond the VM's own resource limits — one shared-cpu-1x VM running both API and worker will be tight under any real concurrent load, fine for a demo |
...
**Total realistic cost**: $0–low single digits/month, mainly if Fly.io's free VM allowance is exceeded. Not literally guaranteed $0 forever, but effectively free at demo traffic.
